    ...
    return ["$scope", '$rootScope', 'ajaxService', '$state', 'getCookieService'
        , function ($scope, $rootScope, ajaxService, $state, getCookieService) {
            ...
            $scope.slides = [
                {
                    active: true,
                    image: 'resources/assets/images/login/banner01.png',
                    id: 0
                },
                {
                    active: false,
                    image: 'resources/assets/images/login/banner02.png',
                    id: 1
                },
                {
                    active: false,
                    image: 'resources/assets/images/login/banner03.png',
                    id: 2
                }
            ];
            ...
            $scope.listenerKey = function(event){
               var keyCode = window.event?event.keyCode:event.which;
                if(keyCode === 13){
                    var element = angular.element(document.getElementById("password"));
                    element[0].focus();
                }
            };
            ...
            $scope.loginSystem = function (user) {
                ...
                ajaxService.AjaxPost(request, "login/login.do").then(function (users) {
                    if (users.status == 1) {
                        ...
                        $state.go('home');
                        /*if($rootScope.user.currentPermission){
                        	$state.go('home');
                        }else{
                        	$state.go('firsthome');
                        }*/
